...
if(isset($_REQUEST['keyword']) and isset($_REQUEST['limit']) and isset($_REQUEST['page'])){
    $url = "https://api.asqtest.ir/manager/doctorList";
    $data=array(
    ...
    echo '<thead>
                <tr>
                    ...
                    <th class="border-b-2 dark:border-dark-5 whitespace-nowrap">تخصص</th>
                   
                    <th class="border-b-2 dark:border-dark-5 whitespace-nowrap"> شماره همراه</th>
                    <th class="border-b-2 dark:border-dark-5 whitespace-nowrap"> پیشنهاد شده</th>
    ...
    foreach ($datas as $data){
        ...
        if($data['active']=='active'){
            $status='<button class=" m-1 p-1 btn-success rounded-lg">فعال</button>';
            $btn='<button onclick="deActive('.$data['id'].')" class=" m-1 p-1 btn-danger rounded-lg">غیرفعال سازی</button>';

        }else{
            $status='<button class="btn-danger m-1 p-1 rounded-lg">غیرفعال</button>';
            $btn='<button onclick="activeTion('.$data['id'].')" class=" m-1 p-1 btn-success rounded-lg">فعال سازی</button>';
        }
        $suggested='پیشنهاد شده';
         if($data['suggested']==0){
             $suggested='پیشنهاد نشده';
         }
        ...
        echo '
         <tr>
                    ...
                    <td class="border-b whitespace-nowrap">'.$data['expertise'].' / '.$data['decExpertise'].'</td>
                    <td class="border-b whitespace-nowrap">'.$data['phone'].' </td>
                    <td class="border-b whitespace-nowrap">'.$suggested.' </td>
                
                    <td class="border-b whitespace-nowrap">'.$data['active'].'</td>
                    <td class="border-b whitespace-nowrap">
                    <a target="_blank" href="dashboard/editeDoctor/'.$data['id'].'" class=" m-1 p-1  rounded-lg btn-warning">ویرایش</a>
                    '.$btn.'                  
                    </td>
